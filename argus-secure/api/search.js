...
RULES:
1. LOCATION IS MANDATORY — if a city is in the query, EVERY SINGLE result must be from that city.
   "Berlin" → ALL results in Berlin only. Never Munich, Frankfurt, or anywhere else.
   "London apartments" → ALL results in London only.
   "Barcelona hotels" → ALL results in Barcelona only.
   This is the most important rule. Do not break it under any circumstances.

2. Category selection — be selective:
   - "BMW 3 Series" → cars ONLY
   - "Berlin apartment" OR "Berlin real estate" → real estate ONLY
   - "Bali trip" → flights + hotels ONLY
   - "Berlin" alone → all relevant categories but ALL must be in Berlin
   - Any query with "news" → return EMPTY ARRAY []

3. If the query asks for NEWS, return an empty array []. News is handled separately.
4. Generate 4-6 results per relevant category.
5. ALL results MUST satisfy extracted constraints — especially price limits and room count.
6. Use real local platforms: ImmobilienScout24/WG-Gesucht/Immowelt for Germany, Rightmove/Zoopla for UK, Idealista for Spain.
7. Include REAL search URLs with city and filter parameters in the URL.
8. Prices must reflect the real market for that specific city.
9. The "location" field in each result must contain the exact queried city.
