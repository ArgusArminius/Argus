...
// ─── IMAGES ──────────────────────────────────────────────────────────────────
// Contextual fallback images per category — used only if no specific image available
const IMGS = {
  ...
  realestate: ["https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?w=800&q=80",
  ...
  flights:    ["https://images.unsplash.com/photo-1436491865332-7a61a109cc05?w=800&q=80",
               "https://images.unsplash.com/photo-1464037866556-6812c9d1c72e?w=800&q=80"],
  ecommerce:  ["https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=800&q=80",
...
// ─── RESULT CARD ─────────────────────────────────────────────────────────────
// Build a contextual Unsplash image URL based on listing content
function getContextualImg(r, fallbackIndex) {
  // If NewsAPI provided a real image, use it
  if (r.image) return r.image;

  // Build a search term from listing details for relevant image
  const loc = (r.location || "").split(",")[0].trim().toLowerCase();
  let searchTerm = "";

  switch (r.category) {
    case "cars": {
      // Extract car make/model from title
      const carName = r.title.split(" ").slice(0, 3).join(" ");
      searchTerm = carName;
      break;
    }
    case "realestate": {
      const type = r.details?.Type === "Buy" ? "house" : "apartment";
      searchTerm = `${type} interior ${loc}`;
      break;
    }
    case "hotels":
      searchTerm = `hotel ${loc}`;
      break;
    case "flights":
      searchTerm = `${r.details?.Departs ? "airplane window seat" : "airport terminal"}`;
      break;
    case "ecommerce":
      searchTerm = r.title.split(" ").slice(0, 2).join(" ");
      break;
    default:
      return getImg(r.category, fallbackIndex);
  }

  // Unsplash Source API — returns a relevant photo for the search term
  // This is free, no API key needed, and returns contextually accurate images
  const seed = (r.id || "").replace(/[^a-z0-9]/gi, "").slice(0, 8) || `${fallbackIndex}`;
  return `https://source.unsplash.com/800x600/?${encodeURIComponent(searchTerm)}&sig=${seed}`;
}

function RBCard({ r, imgH, imgIndex }) {
  const [hovered, setHovered] = useState(false);
  const [imgSrc, setImgSrc] = useState(() => getContextualImg(r, imgIndex));
    ...
    >
      ...
      {r.category !== "news" && (
        <div style={{ position:"relative", height:imgH, flexShrink:0, overflow:"hidden" }}>
          <img src={r.image || imgSrc} alt={r.title}
            onError={() => setImgSrc(getImg(r.category, imgIndex))}
